...
static int CopyPEMRoots(CFDataRef *pemRoots, CFDataRef *untrustedPemRoots, bool debugDarwinRoots) {
	...
	if (debugDarwinRoots) {
		fprintf(stderr, "github.com/palletone/ext-mod/crypto/x509: kSecTrustSettingsResultInvalid = %d\n", kSecTrustSettingsResultInvalid);
		fprintf(stderr, "github.com/palletone/ext-mod/crypto/x509: kSecTrustSettingsResultTrustRoot = %d\n", kSecTrustSettingsResultTrustRoot);
		fprintf(stderr, "github.com/palletone/ext-mod/crypto/x509: kSecTrustSettingsResultTrustAsRoot = %d\n", kSecTrustSettingsResultTrustAsRoot);
		fprintf(stderr, "github.com/palletone/ext-mod/crypto/x509: kSecTrustSettingsResultDeny = %d\n", kSecTrustSettingsResultDeny);
		fprintf(stderr, "github.com/palletone/ext-mod/crypto/x509: kSecTrustSettingsResultUnspecified = %d\n", kSecTrustSettingsResultUnspecified);
	}
		...
		for (j = 0; j < numCerts; j++) {
			...
			if (domains[i] == kSecTrustSettingsDomainSystem) {
				...
				if (debugDarwinRoots) {
					...
					if (errRef != NULL) {
						fprintf(stderr, "github.com/palletone/ext-mod/crypto/x509: SecCertificateCopyShortDescription failed\n");
						CFRelease(errRef);
						continue;
					}
					...
					if (CFStringGetCString(summary, buffer, maxSize, kCFStringEncodingUTF8)) {
						fprintf(stderr, "github.com/palletone/ext-mod/crypto/x509: %s returned %d\n", buffer, (int)result);
					}
					free(buffer);
					CFRelease(summary);
				}
...
func loadSystemRoots() (*CertPool, error) {
	...
	if err == -1 {
		return nil, errors.New("github.com/palletone/ext-mod/crypto/x509: failed to load darwin system roots with cgo")
	}
