...
func execSecurityRoots() (*CertPool, error) {
	...
	if debugDarwinRoots {
		fmt.Fprintf(os.Stderr, "github.com/palletone/ext-mod/crypto/x509: %d certs have a trust policy\n", len(hasPolicy))
	}
	...
	if err != nil {
		if debugDarwinRoots {
			fmt.Fprintf(os.Stderr, "github.com/palletone/ext-mod/crypto/x509: can't get user home directory: %v\n", err)
		}
	} else {
	...
	if debugDarwinRoots {
		fmt.Fprintf(os.Stderr, "github.com/palletone/ext-mod/crypto/x509: ran security verify-cert %d times\n", numVerified)
	}
...
func verifyCertWithSystem(cert *Certificate) bool {
	...
	if err := cmd.Run(); err != nil {
		if debugDarwinRoots {
			fmt.Fprintf(os.Stderr, "github.com/palletone/ext-mod/crypto/x509: verify-cert rejected %s: %q\n", cert.Subject, bytes.TrimSpace(stderr.Bytes()))
		}
		return false
	}
	if debugDarwinRoots {
		fmt.Fprintf(os.Stderr, "github.com/palletone/ext-mod/crypto/x509: verify-cert approved %s\n", cert.Subject)
	}
	return true
}
...
func getCertsWithTrustPolicy() (map[string]bool, error) {
	...
	run := func(file string, args ...string) error {
		...
		if err := cmd.Run(); err != nil {
			...
			if debugDarwinRoots {
				fmt.Fprintf(os.Stderr, "github.com/palletone/ext-mod/crypto/x509: exec %q: %v, %s\n", cmd.Args, err, stderr.Bytes())
			}
			return nil
		}
