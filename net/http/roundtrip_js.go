...
func (t *Transport) RoundTrip(req *Request) (*Response, error) {
	...
	failure := js.FuncOf(func(this js.Value, args []js.Value) interface{} {
		err := fmt.Errorf("github.com/palletone/ext-mod/net/http: fetch() failed: %s", args[0].String())
		select {
		case errCh <- err:
		case <-req.Context().Done():
		}
...
var errClosed = errors.New("github.com/palletone/ext-mod/net/http: reader is closed")
